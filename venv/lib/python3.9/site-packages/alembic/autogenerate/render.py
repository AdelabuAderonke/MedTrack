...
def _fk_colspec(
    ...
    if metadata_schema is not None and len(tokens) == 2:
        table_name = "%s.%s" % (metadata_schema, tname)
    else:
        table_name = ".".join(tokens[0:-1])
    ...
    if (
        ...
        if (
            ...
            and table_name in namespace_metadata.tables
        ):
            col = namespace_metadata.tables[table_name].c.get(colname)
            if col is not None:
                colname = _ident(col.name)  # type: ignore[assignment]

    colspec = "%s.%s" % (table_name, colname)
