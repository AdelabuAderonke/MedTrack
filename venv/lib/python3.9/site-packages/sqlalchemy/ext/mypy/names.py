...
_lookup: Dict[str, Tuple[int, Set[str]]] = {
    ...
    "Relationship": (RELATIONSHIP, _Relnames),
    "RelationshipProperty": (RELATIONSHIP, _Relnames),
    "_RelationshipDeclared": (RELATIONSHIP, _Relnames),
    "registry": (
        REGISTRY,
        {
...
def has_base_type_id(info: TypeInfo, type_id: int) -> bool:
    for mr in info.mro:
        check_type_id, names = _lookup.get(mr.name, (None, None))
        ...
        if isinstance(callee.node, Decorator) and isinstance(
            ...
            if callee.node.func.type and isinstance(
                ...
                if isinstance(ret_type, Instance):
                    return type_id_for_name(ret_type.type.name)
            ...
            if isinstance(target_type, Instance):
                return type_id_for_name(target_type.type.name)
...
def type_id_for_named_node(
    ...
    type_id, names = _lookup.get(node.name, (None, None))

    if type_id is None or names is None:
        return None
...
def type_id_for_name(name: str) -> Optional[int]:
    tokens = name.split(".")
    immediate = tokens[-1]

    type_id, names = _lookup.get(immediate, (None, None))
...
def expr_to_mapped_constructor(expr: Expression) -> CallExpr:
    ...
    column_descriptor.name = NAMED_TYPE_SQLA_MAPPED
