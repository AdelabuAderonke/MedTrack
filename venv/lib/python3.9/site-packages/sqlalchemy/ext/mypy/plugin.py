...
class SQLAlchemyPlugin(Plugin):
    def get_dynamic_class_hook(
        ...
        if names.type_id_for_name(name) is names.DECLARATIVE_BASE:
    ...
    def get_metaclass_hook(
        ...
        if names.type_id_for_name(name) is names.DECLARATIVE_META:
...
def _dynamic_class_hook(ctx: DynamicClassDefContext) -> None:
    ...
    cls.name = ctx.api.qualified_name(ctx.name)
...
def _fill_in_decorators(ctx: ClassDefContext) -> None:
    for decorator in ctx.cls.decorators:
        # set the ".name" attribute of a class decorator
        ...
        if sym and sym.node:
            ...
            if isinstance(sym_type, Instance):
                target.name = f"{sym_type.type.name}.{target.name}"
            else:
